...
def main():
    ...
    port,host=por()
    port=int(port)
    socks.connect((host,port))                                                                                                      #ESTABILSH CONNECTION TO THE SERVER
    if((socks.recv(512))=='s'):                                     
        param_list=pickle.loads(socks.recv(2048))                                                                                   #DESERIALIZE THE TIME DATA
        check_os(param_list)                                                                                                       #CHECK THE TYPE OF OS
        while True:
            s= socks.recv(1024)                                                                                                     #RECEIVE THE SECONDARY DATA
            end_msg+=s
            if not s:
                break
    else:
